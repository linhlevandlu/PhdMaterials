...
During training process, the values of learnable parameters have been updated to increase the accuracy of the network by applying gradient descent \cite{} in backward phase (which algorithm??). Therefore, the networks are designed with a small sharing learning rate and a momentum. Their values are updated over training time to fit with the number of epochs \footnote{An epoch is a single pass through the full training set}.
